...
function AgentMoment({
  ...
  const spokenText = comment
    ?? (isGenerating
      ? "いま、この出来事を振り返っています…"
      : decision?.publicReason
        ? `あの時は、${decision.publicReason}。`
        : "この出来事への振り返りコメントは取得できませんでした。");
  const spokenLabel = comment
    ? "7日後の振り返り"
    : isGenerating
      ? "AFTER INTERVIEW"
      : decision?.publicReason
  ...
  return (
    <div className={`result-agent-moment is-${person}`}>
      <h4>{CHARACTER_NAMES[person]}の選択</h4>
      {decision ? (
        <>
      ...
      <div className={`result-agent-comment is-${person}`} aria-label={`${CHARACTER_NAMES[person]}の振り返りコメント`}>
        <CharacterSprite person={person} className="result-comment-character" />
        <blockquote className={`result-speech-bubble ${comment || decision?.publicReason ? "is-spoken" : "is-status"}`}>
          <strong>{spokenLabel}</strong>
          <p aria-live={isGenerating ? "polite" : undefined}>{comment || decision?.publicReason ? `「${spokenText}」` : spokenText}</p>
        </blockquote>
      </div>
    </div>
  );
          ...
          {highlights.map((highlight, index) => {
            ...
            return (
              <article className="result-highlight-card" key={highlight.id}>
                ...
                {primary ? (
                  <>
                    <div className={`result-highlight-feature ${renderEventCapture ? "is-with-capture" : ""}`}>
                      {renderEventCapture && (
                        <figure className="result-highlight-capture">
                          <div className="result-highlight-capture-frame">
                            {renderEventCapture(primary)}
                          </div>
                          <figcaption>
                            <strong>GAME CAPTURE</strong>
                            <span>{formatEventLocation(primary)}に保存された、その時の二人</span>
                          </figcaption>
                        </figure>
                      )}
