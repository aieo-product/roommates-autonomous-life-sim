...
function safeEmit(emit: Emit, event: StreamEvent): void {
  try {
    ...
    // Result persistence must not depend on an SSE client remaining connected.
  }
}
...
export class GameEngine {
  ...
  async initialize(): Promise<void> {
    ...
    if (stored) {
      ...
      if (
        ...
        this.state.ending &&
        (!this.state.result || this.state.result.status === "generating")
      ) {
  ...
  async resolveTurn(
    ...
    try {
      ...
      emit({ type: "agent.completed", agent: "haru", message: `Haru: ${haru.value.decision}`, data: toPublicDecision(haru.value) });
      emit({ type: "agent.completed", agent: "aoi", message: `Aoi: ${aoi.value.decision}`, data: toPublicDecision(aoi.value) });
  ...
  private async generateResult(terminalState: GameState, emit: Emit): Promise<GameState> {
    ...
    try {
      const producer = buildProducerResult(terminalState.eventLog, terminalState.ending);
      const identity = {
        generationKey: [
          "result",
          terminalState.seed,
      ...
      const reflect = async (
        ...
        failure?: { component: "haru_reflection" | "aoi_reflection"; reason: string; retryable: boolean };
      }> => {
        ...
        if (!this.agents.reflect) {
          return {
            value: {
              ...
              runtime: { source: "fallback", error: "Reflection agent is unavailable" },
            },
            failure: {
              component,
      ...
      safeEmit(emit, {
        type: "warning",
        message: `リザルト生成を完了できませんでした: ${errorMessage(error)}`,
      });
      return this.getState();
    }
  }
