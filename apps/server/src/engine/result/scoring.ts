...
export function buildProducerResult(
  ...
  const scoredEventLog = eventLog.filter(isProducerIntervention);
  const collector = new EvidenceCollector();
  const followUps = refusalFollowUps(scoredEventLog);
  ...
  scoreAgency(scoredEventLog, collector, followUps);
  scoreWellbeing(scoredEventLog, collector);
  // Pacing identifies repeated lock turns used by the care attribution rule.
  scorePacing(scoredEventLog, collector, repeatedLockIds);
  ...
  scoreStory(scoredEventLog, collector, followUps, producerCausedConflicts);

  const evidence = collector.all();
  const axes = buildAxes(evidence);
  const overallScore = axes.reduce((sum, axis) => sum + axis.score, 0);
  const highlights = selectHighlights(scoredEventLog);
