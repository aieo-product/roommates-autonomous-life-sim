...
describe("GameEngine result generation", () => {
  ...
  it("resumes a persisted generating result during initialize and never regenerates an existing ready result", async () => {
    ...
    const resumed = requireReady(resumedEngine.getState());
    expect(resumed.generationKey).toBe(ready.generationKey);
    expect(resumeCoordinator.reflectionCalls).toHaveLength(2);
    expect(resumeRepository.saves.at(-1)?.result?.status).toBe("ready");

    const noOpCoordinator = new ResultCoordinator();
    const readyRepository = new RecordingRepository(resumedEngine.getState());
    const readyEngine = new GameEngine(readyRepository, noOpCoordinator);
    await readyEngine.initialize();

    expect(readyEngine.getState().result).toEqual(resumed);
    expect(noOpCoordinator.reflectionCalls).toHaveLength(0);
    expect(readyRepository.saves).toHaveLength(0);
  });
});
