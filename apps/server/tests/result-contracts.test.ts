...
describe("GameState v2 persistence contracts", () => {
  it("keeps legacy events without conversation while validating new Director exchanges", () => {
    ...
    const conversation = [
      ...
      { speaker: "aoi" as const, text: "朝食のあとで、また話そう。" },
    ];
    const storyBeats = [
      ...
      { kind: "dialogue" as const, actor: "aoi" as const, text: "朝食のあとで、また話そう。" },
    ];
    ...
    expect(
      directorResolvedEventSchema.safeParse({
        ...
        conversation,
        storyBeats: storyBeats.map((beat) =>
          beat.kind === "dialogue" && beat.actor === "aoi"
            ? { ...beat, text: "会話にはない発言" }
            : beat),
      }).success,
    ).toBe(false);
  ...
  it("accepts a loose legacy save and preserves only recognizable public log fields", () => {
    const initial = createInitialGameState("loose-legacy-save");
    const migrated = gameStateSchema.parse({
      ...initial,
      version: 1,
      legacyTopLevelField: "DROP_LEGACY_TOP_LEVEL",
      shared: {
        ...initial.shared,
        legacySharedField: "DROP_LEGACY_SHARED",
      },
      eventLog: [
        {
          ...structuredEvent,
          legacyPrivateField: "DROP_LEGACY_EVENT_PRIVATE",
        },
        { legacyMalformedEvent: true },
      ],
      ending: {
        ...ending,
        legacyEndingField: "DROP_LEGACY_ENDING",
      },
      runtime: {
        haru: {
          source: "app_server",
          threadId: "legacy-haru-thread",
          legacyRuntimeField: "DROP_LEGACY_RUNTIME",
        },
        aoi: { source: "mock" },
        director: { source: "fallback", error: "legacy failure" },
        legacyAgent: { source: "app_server" },
      },
    });

    expect(migrated.version).toBe(2);
    expect(migrated.eventLog).toHaveLength(1);
    expect(migrated.eventLog[0]?.id).toBe(structuredEvent.id);
    expect(migrated.ending).toEqual(ending);
    expect(migrated.runtime.haru).toEqual({
      source: "app_server",
      threadId: "legacy-haru-thread",
    });
    expect(JSON.stringify(migrated)).not.toContain("DROP_LEGACY");
  });
