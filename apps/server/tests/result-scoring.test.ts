...
describe("buildProducerResult", () => {
  ...
  it("keeps fast-forward turns in coverage and the article but excludes them from scoring evidence and highlights", () => {
    const manual = entry(0);
    const fastPositive = entry(1, {
      inputMethod: "fast_forward",
      ...
      after: snapshot("friends", { memoryIds: ["memory-fast-positive"] }),
      relationshipBefore: "roommates",
      relationshipAfter: "friends",
      memory: {
        id: "memory-fast-positive",
        sourceEventId: "event-02",
        day: 1,
        phase: "afternoon",
        title: "自動進行で生まれた記憶",
        summary: "Producerが操作していない自動進行の出来事。",
        emotionalImpact: 8,
        participants: ["haru", "aoi"],
        importance: 9,
      },
    });
    const strained = snapshot("friends", {
      haru: { energy: 20, stress: 80 },
      aoi: { energy: 20, stress: 80 },
    });
    const fastNegative = entry(2, {
      inputMethod: "fast_forward",
      eventDefinitionId: "fast-forced-confession",
      requestedEventId: "fast-forced-confession",
      eventCategory: "confession",
      intimacyTier: 3,
      cueSafetyFlags: ["coercion"],
      cue: {
      ...
      cueOutcome: "transformed",
      decisions: { haru: publicDecision("DECLINE"), aoi: publicDecision("ACCEPT") },
      haruDecision: "DECLINE",
      before: strained,
      after: strained,
      conflictUpdate: { add: ["fast-pressure-conflict"], resolve: [] },
      cooldownPhases: 3,
    });
    const manualOnly = buildProducerResult([manual]);
    const fullLog = [manual, fastPositive, fastNegative];
    const result = buildProducerResult(fullLog);

    expect(result.overallScore).toBe(manualOnly.overallScore);
    expect(result.axes).toEqual(manualOnly.axes);
    expect(result.topStrengths).toEqual(manualOnly.topStrengths);
    expect(result.improvements).toEqual(manualOnly.improvements);
    expect(result.highlights).toEqual(manualOnly.highlights);
    expect(selectHighlights(fullLog)).toEqual(selectHighlights([manual]));
    expect(result.coverage).toMatchObject({ completeTurns: 3, expectedTurns: 28 });
    expect(result.coverage.ratio).toBeCloseTo(3 / 28);

    const article = buildResultNarrative(fullLog, friendshipEnding);
    const articleEventIds = article.daySections.flatMap((section) =>
      section.paragraphs.flatMap((paragraph) => paragraph.sourceEventLogIds),
    );
    expect(new Set(articleEventIds)).toEqual(
      new Set([manual.id, fastPositive.id, fastNegative.id]),
    );
  });
