...
高速スキップは既存の同期Fast Forwardではなく、決定論的ローカル進行のbackground jobとして実行する。job lifecycle、API、checkpoint、cancel、App Server境界は[高速スキップ設計 v1](./fast-skip.md)を正本とする。

- 高速スキップでも各フェーズを通常turnと同じresolverで解決し、全28件のstructured logを残す。
- 実行中は完了フェーズ数、全フェーズ数、最後に保存したDay/phase、cancelを表示する。
- reloadまたはSSE切断後もjobを継続し、保存済み進捗へ再接続する。
- timeline、記事、highlightで自動進行turnを省略しない。ただし「自動進行」バッジを付け、Producer本人が個別に選んだcueと誤認させない。
- 高速スキップはApp Serverを待たずにEndingとログの保存まで完了する。その後のreflectionは通常のResult生成契約で別に進める。
- skip turnはProducer Scoreの直接的な加減点根拠にせず、手動turnとの比率を`standard / assisted / reference`としてランク横へ表示する。
...
- 手動turnと自動進行turnの区別、およびProducer interaction assessment
...
高速スキップは次の独立したjob APIを使う。

- `POST /api/game/skip`: `202 Accepted`とjob ID
- `GET /api/game/skip/:jobId`: checkpoint済み進捗
- `GET /api/game/skip/:jobId/events`: reconnect可能なSSE
- `DELETE /api/game/skip/:jobId`: 次turn開始前のcancel要求

`skip.completed`は対象turnのGameStateとログが保存されたことを表す。Endingへ到達した場合でも、reflectionの成功を待たずに完了させる。UIは完了後に`GET /api/game`で正本stateを再取得し、`result.status`に応じてResultScreenへ切り替える。
...
3. canonical turn pipelineと高速スキップjob、checkpoint、cancel
4. 純粋スコア関数と注目イベント選出、skip採点境界、決定性テスト
5. reflection入力境界、出力schema、公開reaction fallback
6. 事実digest、総集編記事、段落根拠、定型記事fallback
7. `ResultScreen`と総集編/詳細タブ
8. 旧save、途中失敗、再起動、SSE切断、アクセシビリティの統合テスト
...
- グラフなしでも全情報を理解できる
- 高速スキップで完走しても全28ログを保持し、自動進行turnを記事とtimelineから省略しない
- 自動進行turnをProducer本人の加減点根拠にせず、`standard / assisted / reference`を表示する
- skip完了がApp Server reflectionの成功を待たず、partial ResultでもEndingとログを閲覧できる
