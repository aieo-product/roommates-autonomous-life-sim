...
画面上では、PR #13 の`navigator`素材を使う案内役「デコピン」がこの解決結果をプレイヤーへ返す。`resolveSuggestion`が選んだ`SafeSuggestion`を正本とし、Navigator Agentが生成できるのは公開用の短い`message`だけとする。`eventDefinitionId`、`eventTitle`、`outcome`はサーバーが正本から付与し、Agentの文章でイベントを差し替えない。

応答は`navigator.thinking` / `navigator.completed`として配信し、完了時に`GameState.navigator`、`lastEvent.navigatorMessage`、`eventLog.navigatorResponse`へ保存する。追加フィールドは旧セーブ互換のためoptionalとする。Navigator Agentが失敗しても決定的な案内文へfallbackしてターンを続行し、その失敗だけでHaru/Aoi/Directorをfallbackへ切り替えない。`fast_forward`ではNavigator Agentを呼ばず、同じ決定的な案内文を使う。
