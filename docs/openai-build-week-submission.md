...
character reflections, and an evidence-based Dekopin Support Score.
...
changing the rules of the world. Saved names are used consistently in agent
prompts, event dialogue, progress messages, and the final recap.

The Asset Manager exposes the same grid contract used by the game: character
sprite sheets, furniture footprints and pivots, furniture placement, and action
anchors can be replaced without changing the event engine. Roommates Asset Format
v1 additionally validates portable room packs; loading an arbitrary room pack into
the live runtime is a future platform step. Each manual phase advance opens with
an animated time-passage card, and every phase relocates the residents to
furniture-aware starting positions, so the home visibly changes between morning,
...
1. Deterministically resolve the free-form input into a safe event opportunity,
   then validate and freeze the current world state.
2. Start the navigator, Haru, and Aoi concurrently for the same turn. Haru and Aoi
   receive the identical frozen world snapshot; the navigator receives the already
   resolved opportunity and writes the player-facing acknowledgement.
3. Keep Haru's and Aoi's decisions isolated and wait until all three parallel
   results have finished.
4. Ask the Director to reconcile the two resident proposals into one shared event.
...
flowchart TD
    ...
    W -->|"yes"| C["Codex App Server<br/>gpt-5.6-terra"]
    W -->|"no / invalid output"| O{"OpenAI API configured?"}
    O -->|"yes"| G["Responses API<br/>gpt-5.6-terra"]
...
- the navigator writes a friendly acknowledgement for the already resolved,
  bounded event opportunity;
...
- Character names, sprite sheets, and furniture placements are replaceable in the
  live experience; Roommates Asset Format v1 also validates portable room packs.
- Multi-beat scenes move through several action anchors, while animated phase
  transitions make elapsed time and new starting positions visible.
...
5. Watch the separate Navigator, Haru, Aoi, and event progress indicators.
...
9. Review the recap, both reflections, and the evidence behind the Dekopin Support Score.
...
| 2:02–2:24 | Fast Forward and result | “Across seven days, decisions become memories, relationship state, an explainable recap, reflections, and a Dekopin Support Score.” |
| 2:24–2:42 | Runtime panel and architecture | “Codex accelerated the design, implementation, tests, and deployment. GPT-5.6 writes the Navigator acknowledgement, makes independent character decisions, directs events, and writes read-only reflections.” |
