...
- **action tag / anchor ID**: イベントやエージェントが家具を検索し、移動先として選ぶための安定IDです。
...
### 4.5 `anchorIds`（アクションタグ）

`anchorIds`は家具の固定anchor IDであると同時に、イベント生成・移動計画で使うアクションタグです。Assets管理画面では「アクションタグ（カンマ区切り）」として編集します。たとえばアイランドキッチンへ`["kitchen_island", "cook", "prepare_meal"]`を登録すると、runtimeは「料理する」イベントの目的地候補をこれらのIDから検索できます。

各タグはpack内で意味が変わらない安定IDにし、小文字英数字、`.`、`_`、`-`を使います。`anchorIds`を削除・改名すると既存イベントの目的地解決に影響するため、公開後の変更は破壊的変更として扱います。

### 4.6 `orientation`, `flipX`, `flipY`
...
- `role`は`male`, `female`, `navigator`のいずれかで、差し替え可能なキャラクタースロットを表します。
...
`male`と`female`が公開形式の正本です。既存runtimeとの後方互換のため、編集projectでは`male → runtimeId: "haru"`、`female → runtimeId: "aoi"`、`navigator → runtimeId: "navigator"`というエイリアスを任意で保持できます。新しいpackやゲームロジックが`haru`/`aoi`を人物名として扱ってはいけません。

character assetの`label`は画像素材の名前・初期候補です。会話、イベント、リザルトに表示する人物名はキャラクター設定の`characters[slot].profile.name`を正本とし、asset差し替えによって上書きしません。これにより同じ男性・女性スロットへ別デザインを登録しながら、プレイヤーが設定した名前を保てます。

## 6. Room packと配置

roomは整数セル矩形、またはL字などを表現する矩形配列です。zoneを入れ子にできます。同じpack内でroomとzoneのIDは一意です。

ROOMMATES既定2LDKの個室IDは`male_room`と`female_room`を正本とします。新規room pack、配置、exportではこの綴りを使用します。runtime loaderは既存データを読む境界で`haru_room → male_room`、`aoi_room → female_room`、`famale_room → female_room`へ正規化できますが、これら3つのaliasを再出力してはいけません。`famale_room`は互換目的だけの誤記です。独自間取りは名前空間規則に従う別のroom IDを自由に定義できます。
...
projectのfurnitureは公開assetのフィールドに加え、ブラウザpreview用`imageUrl`を持ちます。characterは`file`, `imageUrl`, `portraitFile`, `portraitUrl`, `animationPreset`, `spriteSheet`を持ちます。`imageUrl`はHTTPS URL、同一site相対URL、Blob URL、画像Data URLを利用できますが、公開packへexportするときは必ず相対`file`へ正規化します。

管理画面から書き出す新規projectの個室IDは`male_room`と`female_room`です。旧保存データの`haru_room`, `aoi_room`, 誤記`famale_room`はimport時だけそれぞれcanonical IDへ移行し、新しいexportには残しません。旧projectの`role: "resident"`とsprite sheet metadataなしのcharacterも、`runtimeId`からmale/female slotを推定してv1 editor形式へ自動移行します。
