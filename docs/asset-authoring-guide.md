...
差し替え先は`role: "male"`または`role: "female"`で指定します。既存runtimeとの互換IDはそれぞれ`haru`, `aoi`ですが、新しい公開packでは人物名やasset IDの代わりに使いません。assetの`label`は素材名であり、会話に表示する人物名はゲームのキャラクター設定で管理します。デコピン相当の案内役を作る場合だけ`role: "navigator"`を使います。
...
キャラクターの自律行動用には、room IDと家具の`anchorIds`を安定した公開APIとして扱います。`anchorIds`はAssets管理画面の「アクションタグ（カンマ区切り）」から編集でき、`kitchen_island`, `cook`, `prepare_meal`のように場所と行動の検索語を登録します。画像ファイル名やscreen pxをプロンプト、イベント、移動planへ埋め込まないでください。
...
- characterはrole、portrait、sprite sheetのframe/grid、4方向row、idle/walk列を編集できます。
- 家具の`anchorIds`は「アクションタグ」としてイベントの目的地検索に使われます。
