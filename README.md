...
> **ハッカソン審査員の方へ**
> ROOMMATESは、プレイヤーがキャラクターを操作するのではなく、案内役「デコピン」へ指示し、二人のAIエージェントへ生活上の「きっかけ」だけを届ける自律型恋愛シミュレーションです。HaruとAoiは同じ世界状態から独立して行動を決め、Directorエージェントが二人の意思を尊重しながら出来事を調停します。プレイヤーが望む展開になる保証はありません。
...
flowchart LR
    ...
    API --> N["デコピン / Navigator Agent<br/>案内専用thread"]
    N --> API
...
- [7日間の総集編・Agent感想・デコピンのサポート評価リザルト](./docs/result-experience.md) — 保存契約の`ProducerResult`と互換性を保った、Issue #22のP0設計です。
...
- **デコピン / Navigator thread**: サーバーが安全化・確定したイベントを変更せず、プレイヤーへ短く案内します。
...
各エージェント呼び出しには約15秒のタイムアウトと1回の再試行があります。接続エラー、タイムアウト、不正JSON、スキーマ不一致が起きた役割は、同じインターフェースを持つ決定論的モックへ自動的に切り替わります。そのため、Codex App Serverが利用できない環境でもゲームは最後まで進行できます。
...
| `APP_SERVER_TIMEOUT_MS` | `15000` | 1回のApp Server呼び出しのタイムアウトです。 |
...
- 間取りは24×18タイルの論理座標を1280×720のアイソメトリック表示へ投影しています。現在の家具とキャラクターはSVG/CSSのプレースホルダーで、論理座標を保ったまま最終スプライトへ差し替えられます。
...
npm run check:sites # 公開版の全検証 + パッケージ作成
npm start          # ビルド済みAPIを起動
```

## 公開版の更新

公開先は [ROOMMATES](https://roommates-heart-game.donald-25.chatgpt.site) に固定されています。更新後は次のコマンドで、公開に必要なビルドとパッケージ検証をまとめて実行できます。

```bash
npm run prepare:sites
```

完了後、Codexへ「ROOMMATESを再デプロイして」と依頼すると、同じURLへ新しいバージョンを公開できます。通常の更新で新しいサイトを作る必要はありません。

公開前に型検査と全テストも通す場合は、`npm run check:sites`を使います。
