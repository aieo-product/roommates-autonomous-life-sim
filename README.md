...
> **ハッカソン審査員の方へ**
> ROOMMATESは、プレイヤーがキャラクターを操作するのではなく、設定した二人のAIエージェントへ生活上の「きっかけ」だけを与える自律型恋愛シミュレーションです。男性・女性のキャラクタースロットは同じ世界状態から独立して行動を決め、Directorエージェントが二人の意思を尊重しながら出来事を調停します。プレイヤーが望む展開になる保証はありません。

**「二人を恋に落とすのではない。恋が始まる場所をつくる。」**

## コンセプト

名前・プロフィール・性格・画像を設定した二人が、7日間のルームシェアを送ります。プレイヤーにできるのは、「一緒に夕食を作ってみたら？」「今日は映画を見よう」と提案することだけです。初期packのHaruとAoiは差し替え方法を示すサンプルです。
...
flowchart LR
    ...
    API --> H["Male Character Agent<br/>独立thread"]
    API --> A["Female Character Agent<br/>独立thread"]
...
ゲームエンジンだけが状態を変更できます。各ターンでは、固定した同一スナップショットを男性・女性キャラクターへ同時に渡し、両者の判断が揃ってからDirectorが実際の出来事を決めます。画面・イベント・会話にはCharacter Studioで設定した名前を使います。出力値は検証され、全ステータスは0〜100へ制限されます。

## 設計仕様

- [基本間取りと2Dゲーム画面](./docs/room-layout.md)
- [キャラクタースプライト](./docs/character-sprites.md)
- [キャラクターRuntime契約](./docs/character-runtime-contract.md) — 互換actor ID、公開名、male/female roleの分離。
...
ゲーム画面の「個性設定」から、男性・女性キャラクターの名前、プロフィール、10項目の個性値を個別に編集できます。保存した名前は推論、イベント、会話、進捗、リザルトへ反映されます。個性値は0〜100で検証され、同じ提案でも受諾・修正・拒否・自発行動の傾向や台詞、行動理由へ違いが出ます。
...
ゲーム上部の「Assets」から、家具・キャラクター画像、1×1/1×2等の占有マス、pivot、向き、部屋、floor contactを編集できます。家具のアクションタグはイベントの移動先として使われるため、配置を変えるとキャラクターの行動位置も追従します。設定はブラウザへ保存でき、`roommates.project` JSONとしてimport/exportできます。

公開用packは`npm run validate:assets`で検証します。キャラクターは必ず1×1、冷蔵庫は1×1、対面アイランドキッチンと浴槽は1×2を標準とし、画像サイズではなく論理footprintで衝突と移動を制御します。キャラクター画像は`frameSize`、列・行、4方向row、animation frame列をmanifestへ登録するため、現在のHaru/Aoi素材と同じ契約で任意のsprite sheetへ差し替えられます。

## Codex App Serverをゲームランタイムとして使う

Codex App Serverは開発支援ではなく、ゲーム中の意思決定ランタイムです。

- **Male character thread**: Character Studioの男性キャラクター本人として提案への反応、行動、台詞を決めます。
- **Female character thread**: 相手の判断を先に見ず、女性キャラクター本人として独立に決めます。
...
2. **0:30–1:15 — 独立判断**
   「一緒に夕食を作ってみたら？」と入力します。住人の頭上と画面上部の3段階インジケーターが、設定した二人の名前、できごとの順に進み、キッチンへ注目が移る様子を見せます。
3. **1:15–1:45 — 結果と記憶**
...
6. **2:45–3:00 — App Server runtime**
   デバッグ表示で男性キャラ／女性キャラ／Directorの独立thread IDと、各役割の`app_server`・`openai_api`・`mock`・`fallback`状態を見せます。
...
- 7日間の総集編、注目イベント、二人の振り返り、説明可能なProducer評価リザルト
